...
export default function ProductInventory() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [prods, bills] = await Promise.all([fetchProducts(), fetchBillings()]);
  ...
  const handleAdjust = async (product) => {
    ...
    try {
      ...
      setAdjustMap((prev) => ({ ...prev, [product._id]: { type: "add", qty: "" } }));
      await loadData();
    } catch (e) {
  ...
  // ── Summary totals ──────────────────────────────────────────────────────
  const totalWorth = products.reduce((s, p) => s + (p.price || 0) * (p.currentStock || 0), 0);
  ...
  const LABEL = {
    fontSize: 10.5, color: "#b89060", fontFamily: "Nunito, sans-serif",
    fontWeight: 600, marginBottom: 2, textTransform: "uppercase", letterSpacing: "0.5px",
  };
          ...
          <div className="page-content" style={{ padding: "24px 20px" }}>
            <div className="container-fluid">
              ...
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 10 }}>
                <div>
                  <p className="section-lbl" style={{ marginBottom: 4 }}>Inventory</p>
              ...
              <div className="row g-3 mb-4">
                {[
                  { label: "Total Products", value: products.length, icon: "mdi mdi-package-variant", bg: "#c8965a", textColor: "#5c3d11" },
                  { label: "Total Stock Units", value: totalStockAll.toLocaleString(), icon: "mdi mdi-layers-triple", bg: "#1D9E75", textColor: "#064e3b" },
                  { label: "Total Sold (All Time)", value: totalSoldAll.toLocaleString(), icon: "mdi mdi-cart-check", bg: "#5b6fd6", textColor: "#1e3a8a" },
                  { label: "Inventory Worth", value: `Rs ${totalWorth.toLocaleString()}`, icon: "mdi mdi-cash-multiple", bg: "#e8a020", textColor: "#78350f" },
                ].map((s) => (
                  <div className="col-xl-3 col-sm-6" key={s.label}>
                    <div style={{ background: "#fffdf9", border: "1px solid #e8dcc8", borderRadius: 14, padding: "16px 18px", boxShadow: "0 2px 10px rgba(139,101,50,0.06)", transition: "transform 0.15s" }}
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontSize: 19, fontWeight: 800, color: s.textColor, fontFamily: "Nunito, sans-serif", margin: 0 }}>
                            {s.value}
                          </p>
                        </div>
                      </div>
              ...
              {/* ── Product Cards ── */}
              {filtered.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 20px", color: "#b89060", fontFamily: "Nunito, sans-serif" }}>
                  <i className="mdi mdi-package-variant-closed" style={{ fontSize: 48, display: "block", marginBottom: 12 }} />
                  No products found
                </div>
              ) : (
                <div className="row g-3">
                  {filtered.map((p) => {
                    ...
                    const stockStatus = p.currentStock === 0 ? "danger" : p.currentStock <= 5 ? "warn" : "ok";
                    const stockLabel = p.currentStock === 0 ? "Out of Stock" : p.currentStock <= 5 ? "Low Stock" : "In Stock";
                      ...
                      <div className="col-xl-6 col-12" key={p._id}>
                        <div className="prod-card">
                          ...
                          <div className="prod-card-header">
                            {imgSrc ? (
                              <img src={imgSrc} alt={p.title} className="prod-img"
                                onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }}
                              />
                            ) : null}
                            <div className="prod-img-placeholder" style={{ display: imgSrc ? "none" : "flex" }}>
                              <i className="mdi mdi-book-open-variant" style={{ color: "#8b5e2a", fontSize: 20 }} />
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: "#3d2a10", fontFamily: "Nunito, sans-serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                {p.title}
                              </p>
                              <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap" }}>
                                <span style={{ fontSize: 11, color: "#9a7550", fontFamily: "Nunito, sans-serif" }}>
                                  <i className="mdi mdi-account-edit me-1" />{p.author}
                                </span>
                                <span style={{ fontSize: 11, color: "#9a7550", fontFamily: "Nunito, sans-serif" }}>
                                  <i className="mdi mdi-tag-outline me-1" />{p.category}
                                </span>
                                <span style={{ fontSize: 11, color: "#9a7550", fontFamily: "Nunito, sans-serif" }}>
                                  <i className="mdi mdi-barcode me-1" />{p.barcode}
                                </span>
                              </div>
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                              ...
                              <span className={`warm-badge badge-${stockStatus}`}>{stockLabel}</span>
                            </div>
                          </div>
                          ...
                          <div className="stat-boxes">
                            ...
                            <div className="stat-box">
                              <p style={LABEL}>
                                <i className="mdi mdi-cart-plus me-1" />Total Products
                              </p>
                              <p className="stat-box-val val-blue">{p.currentStock}</p>
                              <p style={{ fontSize: 10, color: "#94a3b8", fontFamily: "Nunito, sans-serif", margin: "2px 0 0" }}>
                                currently available
                              </p>
                            </div>
                            ...
                            <div className="stat-box">
                              <p style={LABEL}>
                                <i className="mdi mdi-plus-circle-outline me-1" />Added
                              </p>
                              <p className="stat-box-val val-green">{p.initialStockQuantity || p.totalPurchased}</p>
                              <p style={{ fontSize: 10, color: "#94a3b8", fontFamily: "Nunito, sans-serif", margin: "2px 0 0" }}>
                                qty added initially
                              </p>
                            </div>
                            ...
                            <div className="stat-box">
                              <p style={LABEL}>
                                <i className="mdi mdi-cart-arrow-right me-1" />Total Sold
                              </p>
                              <p className="stat-box-val val-amber">{p.totalSold}</p>
                              <p style={{ fontSize: 10, color: "#94a3b8", fontFamily: "Nunito, sans-serif", margin: "2px 0 0" }}>
                            ...
                            <div className="stat-box">
                              <p style={LABEL}>
                                <i className="mdi mdi-warehouse me-1" />Remaining
                              </p>
                              <p className="stat-box-val val-teal">{p.currentStock}</p>
                              <p style={{ fontSize: 10, color: "#94a3b8", fontFamily: "Nunito, sans-serif", margin: "2px 0 0" }}>
                                in stock now
                              </p>
                            </div>
                          ...
                          {isAdmin && (
                            <div className="adj-row">
                              <span style={{ fontSize: 12, color: "#9a7550", fontFamily: "Nunito, sans-serif", fontWeight: 700 }}>
                                Adjust Stock:
                              </span>
                              ...
                              <button
                                ...
                                onClick={() => setAdjustMap(prev => ({ ...prev, [p._id]: { ...getAdj(p._id), type: "add" } }))}
                              >
                                <i className="mdi mdi-plus me-1" />Add
                              </button>
                              <button
                                className={`adj-type-btn sub ${adj.type === "sub" ? "active" : ""}`}
                                onClick={() => setAdjustMap(prev => ({ ...prev, [p._id]: { ...getAdj(p._id), type: "sub" } }))}
                              >
                                <i className="mdi mdi-minus me-1" />Subtract
                              </button>
                              ...
                              <input
                                ...
                                placeholder="Qty"
                                value={adj.qty}
                                onChange={(e) => setAdjustMap(prev => ({ ...prev, [p._id]: { ...getAdj(p._id), qty: e.target.value } }))}
                              />
                              ...
                              <span style={{ marginLeft: "auto", fontSize: 11, color: "#b89060", fontFamily: "Nunito, sans-serif" }}>
                                Worth: Rs {(Number(p.price) * p.currentStock).toLocaleString()}
                              </span>
                            </div>
                          )}
                        </div>
